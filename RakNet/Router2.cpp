...
void Router2::OnRequestForwarding(Packet *packet)
{
	...
	if (result==UDPFORWARDER_FORWARDING_ALREADY_EXISTS)
	{
		...
		SendForwardingSuccess(packet->guid, endpointGuid, forwardingPort);
	}
	...
	while (i < miniPunchesInProgress.Size())
	{
		if (miniPunchesInProgress[i].sourceGuid==packet->guid || miniPunchesInProgress[i].endpointGuid==packet->guid)
		{
				...
				miniPunchesInProgress[i].gotReplyFromSource==true)
			{
				...
				SocketLayer::Instance()->SendTo_PC( miniPunchesInProgress[i].forwardingSocket, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), miniPunchesInProgress[i].endpointAddress.binaryAddress, miniPunchesInProgress[i].endpointAddress.port );

				SendForwardingSuccess(miniPunchesInProgress[i].sourceGuid, miniPunchesInProgress[i].endpointGuid, miniPunchesInProgress[i].forwardingPort);
				miniPunchesInProgress.RemoveAtIndexFast(i);
			}
			else
			{
