...
//		DataStructures::DefaultIndexType oldSize = forwardList.GetSize();
		...
		return SocketLayer::GetLocalPort ( fe->socket );
	}

	return SocketLayer::GetLocalPort ( forwardList[insertionIndex]->socket );
}
								  ...
								  unsigned short *forwardingPort, SOCKET *forwardingSocket)
{
	...
	*forwardingPort = AddForwardingEntry(srcAndDest, timeoutOnNoDataMS, forceHostAddress);
